...
function goGame() {
    var score = 0, life = 30, level, levelEnemiesDone, running, count, last, secondToLast, nextAction, enemies, music, highScore, fireP, fireCount;
    var viewWidth = 960;
    var viewHeight = 540;
    ...
    // fire renderer method
    function drawFire(x, y) {
        log("Hunter fire", x, y)
        fireP = new PIXI.Graphics();
        fireP.lineStyle(3, 0x555555);
        fireP.moveTo(480, 540);
        fireP.lineTo(Math.round(x), Math.round(y));
        fireCounter = 150;
        stage.addChild(fireP);
        log(fireP);
    }
    ...
    function levelScreen() {
        ...
        if (fireP && fireP.stage) {
            stage.removeChild(fireP);
        }
        stage.addChild(levelText);
        stage.addChild(actionButton);
...
1
    ...
    function createEnemy() {
        ...
        enemy.mousedown = enemy.touchstart = function(data) {
            ...
            if (this.status !== "dying") {
                ...
                drawFire(this.position.x, this.position.y);
                createEnemy(); // another
            }
        };
    ...
    function animate() {
        ...
        // manage player's shot
        if (fireP) {
            log("firrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr", fireP);
            if (fireCount) {
                fireCount--;
            } else {
                stage.removeChild(fireP);
